...
#define kRefreshRate (15.0f / 1000.0f)
...
static const CGFloat horizontalHeaderHeight = 80.0f;
static const CGFloat verticalHeaderWidth = 80.0f;
...
- (UIImage *)generateStaticImage {
  ...
  CGFloat topGutterHeight = 200.0f;
  CGFloat rightGutterWidth = 200.0f;
    ...
    if (CGRectContainsPoint(grid, location)) {
      ...
      if (([self.patchbayDelegate isSourceDeviceInBounds:srcIndex]) &&
        ...
        if ([self.patchbayDelegate isPatchedFrom:srcIndex to:dstIndex]) {
          ...
          [self.patchbayDelegate setPatchedFrom:empty to:dstIndex];
        } else {
          [self.patchbayDelegate setPatchedFrom:srcIndex to:dstIndex];
        }
      }
    }
  }
